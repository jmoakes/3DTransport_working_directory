 ...
 * Jessica M. Oakes edited on: 4/5/16
 */ 
...
int main (int argc, const char * argv[]) {
    double t1, t2, runtime, nextoutputtime, firstoutputtime, time_release[5][100000];
	int    df, ss, ii, jj, ee, i, j, k, startindex, num_integrated, outputframe = 0 , outputcount = 0, num_particles = 0;
	...
	if(FTLE_Compute) {
		...
		if(Int_NormalFlow > 0 && Particle_Radius > TINY)
	...
	for(df = Data_FirstFrame; df != Data_LastFrame; df = df + Int_TimeDirection) {
        ...
        if(FTLE_Compute) {
						   ...
						   for(j = 0; j < FTLE_CartMesh.YRes; j++) {
							   for(i = 0; i < FTLE_CartMesh.XRes; i++) {
								   ...
								   if(FTLE_MeshPt[i][j][k].Pt.LeftDomainTime > -TINY && (!FTLE_MeshPt[i][j][k].Pt.LeftDomain || Int_Extrapolate)) {
									   ...
									   Advect(&TempPoint, 0, t1, t2, ss, 0, 0, 0);
									   num_integrated++;
			...
			if(Trace_ReleaseStrategy == STAGGERED) {
						...
						if(rnode->slide.Status != COMPLETE &&
						   (rnode->slide.Status == LAUNCHED || (rnode->slide.StartTime >= Data_LoadedTMin && rnode->slide.StartTime <= Data_LoadedTMax))) {
							...
							if(!rnode->pt.LeftDomain || Int_Extrapolate) {
								...
								t1 = fmax(rnode->slide.StartTime, Data_LoadedTMin);
								/*test[num_integrated] = t1;*/

								/*printf("%3.2e ", test[num_integrated]);*/
								
								/*printf("%d", Trace_NumReleaseLocations);*/
								/*printf("%g", t1);*/
								/*printf("%d", startindex);*/
								/*printf("%d", num_integrated);*/
				...
				printf("%3.2e \t", time_release[1][10000]);	
				printf("%3.2e \t", time_release[1][60000]);	
				...
				/*fprintf(stderr, "%G ",  rnode->pt.X[0]);*/
    			/*fprintf(stderr, "%G ", pt.X[0]);*/

				/*for(ss = 0; ss < Trace_NumReleaseLocations; ss++) {
					test[ss] = rnode->slide.StartTime;
				}*/
				/*fprintf(stderr, "%G ", ParticleHitWall[0][1000]);*/
						   ...
						   (Trace_Launches[ss].Status == LAUNCHED || (Trace_Launches[ss].StartTime <= fmax(t1, t2) && Trace_Launches[ss].StartTime >= fmin(t1, t2)))) {
							...
							for(ii = 0; ii < Trace_NumTracers; ii++) {
								if(Trace_MeshPt[ii].LeftDomainTime > -TINY && (!Trace_MeshPt[ii].LeftDomain || Int_Extrapolate)) {
									Advect(&Trace_MeshPt[ii], rnode->slide.StartTime, t1, t2, num_integrated, rnode->pt.X[0], rnode->pt.X[1], rnode->pt.X[2]);
									num_integrated++;
								}
	...
	FILE *fp3;
	fp3=fopen("particle_hit_wall.txt", "w");
	for (i = 0; i < HitWall_Index; i++){
    	fprintf(fp3, "%G \t", ParticleHitWall[i][0]);
    	fprintf(fp3, "%5.5e \t", ParticleHitWall[i][1]);
    	fprintf(fp3, "%5.5e \t", ParticleHitWall[i][2]);
    	fprintf(fp3, "%5.5e \t", ParticleHitWall[i][3]);
    	fprintf(fp3, "%5.5e \t", ParticleHitWall[i][4]);
    	fprintf(fp3, "%5.5e \t", ParticleHitWall[i][5]);
    ...
    for (i=0;i<ss;++i)
		{ free(ParticleHitWall[i]);}
	free(ParticleHitWall);	

	fprintf(stderr, "Number of tracers integrated: %d \n", num_integrated);
	fprintf(stderr, "Number of launch times: %d \n", num_particles);

    fprintf(stderr,"YOU ARE HERE \n");
    FILE *fp2;
	fp2=fopen("time_release.txt", "w");
	for (i = 0; i < num_particles; i++){
		fprintf(fp2, "%G \t", time_release[0][i]);	
		fprintf(fp2, "%5.5e \t", time_release[1][i]);	
		fprintf(fp2, "%5.5e \t", time_release[2][i]);
		fprintf(fp2, "%5.5e \t", time_release[3][i]);	
		fprintf(fp2, "%5.5e \t", time_release[4][i]);
		fprintf(fp2,"\n");
	}
	fclose(fp2);
    
	/*printf("%2.3e", rnode->slide.StartTime);*/
	fprintf(stderr,"%3.2e ", time_release[1][10]);
	fprintf(stderr,"%3.2e ", time_release[1][4000]);
